...
function WarnPerformanceThreshold(threshold: number = 100, performanceHandler?: (executionTime: number, methodName: string) => void): MethodDecorator {
    ...
    return function(target: Object, propertyKey: string | symbol, descriptor: TypedPropertyDescriptor<any>) {
        ...
        if (typeof descriptor.value !== 'function') {
            throw new Error('🐞 [WarnPerformanceThreshold] Can only be applied to methods.');
        }
        ...
        descriptor.value = function(...args: any[]) {
            ...
            if (isNodeEnvironment && start !== undefined) {
                ...
                console.error('🐞 [WarnPerformanceThreshold] Performance timing not supported in this environment.');
                return result;
            }

            if (executionTime > threshold) {
                const warningMessage = `⚠️ [Performance Warning] ${target.constructor.name}.${String(propertyKey)} exceeded threshold of ${threshold} ms`;
